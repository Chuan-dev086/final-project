...
<body>
    <div class="container my-4">
        ...
        <div class="glass-panel">
            <div class="mb-4">
                <h2 class="mb-1 gradient-title"><i class="me-2 bi bi-shield-lock-fill"></i>User Management</h2>
            </div>
            ...
            <div class="table-responsive">
                <table class="mb-0 table table-glass align-middle">
                    ...
                    <tbody>
                        <?php if (empty($users)): ?>
                            <tr>
                                <td colspan="5" class="py-4 text-center text-white-50">
                                    <i class="d-block mb-2 fs-3 bi bi-people"></i>No users found.
                                </td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($users as $row): ?>
                                <tr>
                                    ...
                                    <td><span class="custom-album-name"><i class="me-2 bi bi-person-circle small"></i><?= $row['username'] ?></span></td>
                                    <td><span class="custom-date-row"><?= $row['email'] ?></span></td>
                                    <td>
                                        <?php switch ($row['role']):
                                            case 'Admin': ?>
                                                <span class="badge-admin role-badge">
                                                    <i class="me-1 bi bi-shield-fill-check"></i>Admin
                                                </span>
                                                <?php break; ?> // 💡 记得加上 break 防止代码击穿

                                            <?php
                                            case 'Manager': ?>
                                                <span class="badge-manager role-badge">
                                                    <i class="me-1 bi bi-person-workspace"></i>Manager
                                                </span>
                                                <?php break; ?>
                                            ...
                                            ?>
                                                <span class="badge-user role-badge">
                                                    <i class="me-1 bi bi-person-fill"></i>User
                                                </span>
                                        <?php
                                                break;
                                        endswitch;
                                    ...
                                    <td class="text-center">
                                        <a href="./pages/edit-user.php?id=<?= $row['id'] ?>" class="btn-edit">
                                            <i class="me-1 bi bi-pencil-square"></i>Edit
                                        </a>

                                        <!-- 防自杀机制：如果是当前登录的自己，不允许删除 -->
                                        <?php if ($row['id'] != $_SESSION['user_id']): ?>
                                            <a href="./actions/delete-users.php?id=<?= $row['id'] ?>"
